...
class AvlRankTree {
	 ...
	 */
	void insertionHelper(const std::shared_ptr<T> &inData, AvlNode<T> *&insertionPoint);
...
public:
	 ...
	 */
	void insert(const std::shared_ptr<T> &inData);
	 ...
	 */
	const std::shared_ptr<T> &find(const T &data) {
		// return findHelper(data, root);

		AvlNode<T> *current = root;
		// Static empty ptr to return by reference if not found
		static const std::shared_ptr<T> empty = nullptr;
		...
		while (current != nullptr) {
			if (*(current->data) > data) {
				...
				// Found! Return reference to the node's stored ptr.
				// Zero atomic ops.
				return current->data;
			}
		}
	...
	* @return If found shared_ptr to the requested data. else nullptr.
	*/
	const std::shared_ptr<T>& findRank(int i);
...
template<class T>
void AvlRankTree<T>::insertionHelper(const std::shared_ptr<T> &inData, AvlNode<T> *&insertionPoint) {
...
template<class T>
void AvlRankTree<T>::insert(const std::shared_ptr<T> &inData) {
	if (root == nullptr) {
...
//
// 		if (sizeLeft > i) {
// 			//if left sub tree had more then i - 1 nodes. the node we are looking for is in the left size
// 			current = current->left;
// 		} else if (current->right != nullptr) {
// 			// if left sub tree doesnt have enough nodes, the one we are looking for is in the rights size
// 			current = current->right;
// 			i -= sizeLeft;
// 		}
// 	}
...
template<class T>
const std::shared_ptr<T>& AvlRankTree<T>::findRank(int i) {
	static const std::shared_ptr<T> empty = nullptr;
